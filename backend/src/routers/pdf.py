...
from src.routers.pdf_schemas import (
    ErrorResponse,
    HTTPValidationError,
    PDFDeleteResponse,
    PDFListResponse,
...
async def upload_pdf(
    ...
    if processing_state in ["summary_generation", "embeddings_complete"]:
        logger.info(f"File partially processed (summary stage): {file.filename}")
        return JSONResponse(
            status_code=200,
            content={
                "message": "File exists, summary generation in progress or incomplete. Processing will continue from summary generation.",
                "filename": file.filename,
                "notebook": notebook_name,
    ...
    if processing_state in ["processing", "uploaded"]:
        logger.info(f"File partially processed (embeddings stage): {file.filename}")
        return JSONResponse(
            status_code=200,
            content={
                "message": "File exists, embeddings generation in progress or incomplete. Processing will continue from embedding generation.",
                "filename": file.filename,
                "notebook": notebook_name,
                "processing_state": processing_state,
            },
