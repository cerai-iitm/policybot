...
const PolicyCollectionsSection = ({ notebooks, active, onSelect }: Props) => {
 const getNotebookImage = (title?: string) => {
  ...
  if (t.includes("education") || t.includes("academic")) return EducationImg.src
  if (t.includes("digital") || t.includes("governance") || t.includes("cyber") || t.includes("it")) return AiImg.src
  if (t.includes("union") || t.includes("budget")) return UnionBudgetImg.src
